...
t_cell	game_get_cell(t_game *game, t_int x, t_int y)
{
	ft_assert (x >= 0 && x < game->width,
		"game_get_cell: x (%i) is out of bounds %i.", x, game->width);
	ft_assert (y >= 0 && y < game->height,
		"game_get_cell: y (%i) is out of bounds %i.", y, game->height);
...
{
	ft_assert (x >= 0 && x < game->width,
		"game_set_cell: x (%i) is out of bounds %i.", x, game->width);
	ft_assert (y >= 0 && y < game->height,
		"game_set_cell: y (%i) is out of bounds %i.", y, game->height);
	if (game_get_cell (game, x, y) == CELL_COLLECTIBLE)
		game->collectibles -= 1;
	...
	while (y < game->height)
	{
		...
		while (x < game->width)
		{
			...
			if ((x == 0 || x == game->width - 1
					|| y == 0 || y == game->height - 1) && cell != CELL_WALL)
